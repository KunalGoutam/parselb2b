...
var inventoryController = myApp.controller('PickedUpColdStorageController', ['$scope', 'ShipmentService', 'DriversService', 'CONSTANT', '$rootScope',
    function($scope, ShipmentService, DriversService, CONSTANT, $rootScope) {
        ...
        $scope.downloadPickupXls = function() {
            
            var link = document.createElement("a");
            var fromDate = formatDate($scope.date.fromDate);
            window.open($rootScope.baseUrl + '/adm/csitemsreport/' + fromDate + '/' + $scope.selectedSlot.slot.startTime + '-' + $scope.selectedSlot.slot.stopTime + '/' + 1);
        };
